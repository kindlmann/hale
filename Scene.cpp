...
Scene::~Scene() { /* anything to clean up? */ }

void
Scene::add(const Polydata *pd) {
    _polydata.push_back(pd);
}

void
Scene::bgColor(float rr, float gg, float bb) {
    ELL_3V_SET(_bgColor, rr, gg, bb);
}
...
void
Scene::lightDir(glm::vec3 dir) {
    _lightDir = glm::normalize(dir);
}
glm::vec3
Scene::lightDir() const {
    return _lightDir;
}
...
void
Scene::bounds(glm::vec3 &finalmin, glm::vec3 &finalmax) const {
    glm::vec3 min, max;
...
Scene::draw() {
    ...
    if (debugging) printf("# glClear(GL_DEPTH_BUFFER_BIT);\n");
    glClear(GL_COLOR_BUFFER_BIT);
    if (debugging) printf("# glClear(GL_COLOR_BUFFER_BIT);\n");
