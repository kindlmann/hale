...
Polydata::_buffer(bool newaddr) {
    ...
    if (ibits & (1 << limnPolyDataInfoNorm)) {
        ...
        if (newaddr) {
            ...
            if (debugging)
                printf(
                  "# glBufferData(GL_ARRAY_BUFFER, %u, lpd->norm, GL_DYNAMIC_DRAW);\n",
                  (unsigned int)(lpd->normNum * sizeof(float) * 3));
        } else {
            glBufferSubData(GL_ARRAY_BUFFER, 0, lpd->normNum * sizeof(float) * 3,
                            lpd->norm);
        }
    ...
    if (ibits & (1 << limnPolyDataInfoRGBA)) {
        ...
        if (newaddr) {
            ...
            if (debugging)
                printf(
                  "# glBufferData(GL_ARRAY_BUFFER, %u, lpd->rgba, GL_DYNAMIC_DRAW);\n",
                  (unsigned int)(lpd->rgbaNum * sizeof(char) * 4));
        } else {
            glBufferSubData(GL_ARRAY_BUFFER, 0, lpd->rgbaNum * sizeof(char) * 4,
                            lpd->rgba);
        }
...
Polydata::Polydata(limnPolyData *poly, bool own, const Program *prog, std::string name) {
...
void
Polydata::colorSolid(float rr, float gg, float bb) {
    _colorSolid = glm::vec4(rr, gg, bb, 1.0f);
}
void
Polydata::colorSolid(glm::vec3 rgb) {
    _colorSolid = glm::vec4(rgb, 1.0f);
}
void
Polydata::colorSolid(glm::vec4 rgba) {
    _colorSolid = rgba;
}
glm::vec4
Polydata::colorSolid() const {
    return _colorSolid;
}

void
Polydata::program(const Program *prog) {
...
const Program *
Polydata::program() const {
    return _program;
}

void
Polydata::bounds(glm::vec3 &finalmin, glm::vec3 &finalmax) const {
...
Polydata::draw() const {
    ...
    for (unsigned int ii = 0; ii < lpld->primNum; ii++) {
        glDrawElements(Hale::limnToGLPrim(lpld->type[ii]), lpld->icnt[ii],
