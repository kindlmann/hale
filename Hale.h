...
typedef void (*Viewer3Refresher)(void *);
...
/* utils.cpp */
extern int buttonIdx(int button, int mods);
extern void init();
extern void done();
...
class Camera {
  public:
    ...
    void init(glm::vec3 from, glm::vec3 at, glm::vec3 up, double fov, double aspect,
...
/* Viewer3.cpp: Viewer3 contains and manages a GLFW window, including the
   camera that defines a 3D view within the viewer.  We intercept all
   the events in order to handle how the camera is updated */
class Viewer3 {
  public:
    explicit Viewer3(int width, int height, const char *label, Scene *scene);
    ~Viewer3();
    ...
    void refreshCB(Viewer3Refresher cb);
    Viewer3Refresher refreshCB();
  ...
  protected:
    ...
    Viewer3Refresher _refreshCB;
    void *_refreshData;

    int _pixDensity, _widthScreen, _heightScreen, _widthBuffer, _heightBuffer;
...
class Program {
  public:
    ...
    void uniform(std::string, glm::mat3, bool sticky = false) const;
...
extern void uniform(std::string, glm::mat3, bool sticky = false);
...
extern std::map<std::string, glm::mat3> stickyUniformMat3;
...
class Polydata {
  ...
  protected:
    ...
    glm::mat3 _model3IT;        // inverse transpose of upper 3x3 of _model
...
**
** One principle is that the Scene is oblivious to the Viewer3: so the light
